...
document.addEventListener("DOMContentLoaded", () => {
    const novaPoshta = document.getElementById("nova-poshta");
    const ukrPoshta = document.getElementById("ukr-poshta");
    const pickup = document.getElementById("pickup");
    const addressContainer = document.getElementById("address-container");
    const departmentInput = document.getElementById("department");
    const fullNameInput = document.getElementById("full_name");
    const phoneForDeliveryInput = document.getElementById("phone_delivery");
    const cityInput = document.getElementById("city");

    function toggleAddressInput() {
        if (novaPoshta.checked || ukrPoshta.checked) {
    ...
    novaPoshta.addEventListener("change", toggleAddressInput);
    ukrPoshta.addEventListener("change", toggleAddressInput);
    pickup.addEventListener("change", toggleAddressInput);
    ...
    //     const address = `${formData.get('department')}, ${formData.get('full_name')}, ${formData.get('phone_delivery')}, ${formData.get('city')}`;
    //     const details = formData.get('details');

    //     const url = `http://192.168.0.106:5000/order/${name}/${phone}/${delivery}/${address}/${details}`;
	...
	orderForm.addEventListener('submit', (event) => {
		...
		} else if (delivery === 'ukr-poshta') {
			delivery = 'Укр Почта';
		} else if (delivery === 'pickup') {
			delivery = 'Самовывоз';
		}
	
		const url = 'http://192.168.0.106:5000/order';
	
		console.log('Отправка данных:', {name, phone, delivery, address, details});
